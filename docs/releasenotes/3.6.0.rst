...
to how custom libraries are imported in Robot Framework. If the the file has the same name as transformer it will
be auto imported::
...
will import TransformerB and TransformerA (it doesn't need to be defined in ``__init__.py`` but imported here).
