...
For example, you can have the keyword that you want to to align automatically (using ``AlignKeywordsSection``) and
...
separetely from separator (``--spacecount``). Continuation indent ``--continuation-indent`` allow you define indent
...
Fixes
-----
- documentation now properly states that ``NormalizeTags`` is default transformer (`#335 <https://github.com/MarketSquare/robotframework-tidy/issues/335>`_)
...
- Transformers inherits now from Robotidy internal ``Transformer`` class rather than ``ModelTransformer``
