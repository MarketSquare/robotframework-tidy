...
class TestOrderSettingsSection:
    ...
    def test_empty_group_order(self):
        run_tidy_and_compare(
            self.TRANSFORMER_NAME, source="test.robot", expected="test_empty_group_order.robot", config=":group_order="
        )
    ...
    def test_remote_library_as_external(self):
        run_tidy_and_compare(
            self.TRANSFORMER_NAME, source="remote_library.robot", config=":imports_order=library,resource,variables"
        )
